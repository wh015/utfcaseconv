...
template <typename IT> size_t codepoint_32to8(char32_t cdpt, IT &dst)
{
    if (cdpt <= 0x7F)
    {
        *dst++ = (char) (cdpt);
...
template <typename IT> inline char32_t codepoint_8to32(IT &begin, IT end) noexcept
{
...
template<typename IT, typename IT2> inline size_t toupper(IT begin, IT end, IT2 dst)
{
    auto res = 0;
    while (begin != end) {
...
template<typename IT, typename IT2> inline size_t tolower(IT begin, IT end, IT2 dst)
{
    auto res = 0;
    while (begin != end) {
        auto cdpt = codepoint_8to32(begin, end);
